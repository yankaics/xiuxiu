...
function remove_this_photo(inputid,obj)
{
	remove_this(obj);
	var count = $('#'+inputid+'_images ul').length;
	$('#photo_count').html(count);
}

function up_photoes(iframeWin, topWin,id,inputid){ 
		var data = '';
		var aidinput='';
		var num = iframeWin.$('#myuploadform > div').length;
		if(num){
			iframeWin.$('#myuploadform  div ').each(function(){
					...
					var name = $(this).find('#namedata').val();
					//if(status==0) aidinput = '<input type="hidden" name="aid[]" value="'+aid+'"/>';
					data += '<ul id="uplist_'+aid+'">'+aidinput;
					data += '<input type="hidden" name="aid[]" value="'+aid+'"  /> ';
					data += '<input type="hidden" name="images[]" value="'+src+'"  /> ';
					data += '<input type="hidden"   name="names[]" value="'+name+'"  />';
					data += '<li class="item-title"><img src="'+src+'" width="100" height="100" /></li>';
					data += '<li class="item-size">'+name+'</li>';
					data += '<li class="item-progress">已完成</a>';
					data += '<li class="item-directory">'+document.getElementById('photo_type').value+'</li>';
					data += '<li class="item-status"><a href="javascript:remove_this_photo(\''+inputid+'\',\'uplist_'+aid+'\');">移除</a></li>'
					data += '</ul>';
			});			
			$('#'+inputid+'_images').append(data);
			var current_count = $('#current_count').val();
			$('#photo_count').html(current_count+num);
		}
}
