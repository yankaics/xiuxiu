...
class BaseAction extends Action
{
    ...
    public function _initialize() {
			...
			if($_COOKIE['YP_auth']){
				...
				$this->_groupid = $_COOKIE['YP_groupid']=6;
				$this->_userid =0;
			}
			...
			if(GROUP_NAME=='User')
            {
                if($this->_userid>0)
                {
                    ...
                    {
                        $this->assign('realname',$data['orgname']);
                    }
                    else
                    {
                        $this->assign('realname',$data['realname']);
                    }
                    if(!$data['avatar'])
                    {
                        if($this->isCompanyMember())
                        {
							...
							//$this->assign('avatar','../Public/images/ogsimg_03.jpg');
                        }
                        else
                        {
                            //$this->assign('avatar','../Public/images/ogsimg_03.jpg');
    ...
    protected function isModelMember()
    {
        $result = ($this->_groupid == 3)?true:false;
        return $result;
    }

    protected function  isCompanyMember()
    {
        $result = ($this->_groupid == 4)?true:false;
        return $result;
    }

    protected function  isGuestMember()
    {
        $result = ($this->_groupid == 6)?true:false;
        return $result;
    }
    protected function _list($where='',$fields='*')
    {
    	import ('@.ORG.Page');
    	$mod = $this->dao;
