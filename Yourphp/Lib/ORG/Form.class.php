...
class Form extends Think {
	...
	public function datetime($info,$value){
        ...
        $fmt = "Y-m-d H:i:s";
        if($info['setup']['format'])
        {
        	$fmt = $info['setup']['format'];
        }
        $fmt2 = "yyyy-MM-dd HH:mm:ss";
        if($info['setup']['format2'])
        {
        	$fmt2 = $info['setup']['format2'];
        }
		$value = $value ?  toDate($value,$fmt) : toDate(time(),$fmt);

		$parseStr = '<input  class="Wdate input-text  '.$info['class'].'"  '.$validate.'  name="'.$field.'" type="text" id="'.$id.'" size="25" onFocus="WdatePicker({dateFmt:\''.$fmt2.'\'
		})" value="'.$value.'" />';
        return $parseStr;
	}
